...
from src import constants, data_generator
...
def get_data_iterator(data_filename, data_config, vocab_lookup_ops, batch_size, num_epochs, is_train):
  with tf.device('/cpu:0'):
    ...
    # get the dataset
    dataset = tf.data.Dataset.from_generator(lambda: data_generator.conll_data_generator(data_filename, data_config),
    ...
    # else:
    #   # just do normal batching, so that dataset ordering is maintained
    #   # todo resolve this w/ doc/sentence ids so that eval can be faster
